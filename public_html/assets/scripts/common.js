    ...
    if (Modernizr.webgl) {
        ...
        $.get("/assets/scripts/markers.json", function(data) {
            $.each(data, function(index, val) {
                ...
                var marker = WE.marker(val.point, "/assets/images/common/mapicons01-131.png", 32, 32).addTo(earth);

                marker.bindPopup('<dl class="popup"><dt><a target=_blank href="' + val.url + '">' + val.name + '</a></dt><dd>' + val.detail + '</dd></dl>', {
                    maxWidth: 120,
                    closeButton: true
                });
                if (val.show_popup) {
        ...
        // earth.setView([35.38, 139.42], 2); //with out animation (auto rotation)
